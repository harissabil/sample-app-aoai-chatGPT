...
class MultiSourceSearch:
    ...
    async def search_content_filtered(self, keyword: str, document_list: List[str] = None, k: int = 10) -> Dict[str, Any]:
        """Search content with optional document filtering"""
        try:
            filter_expression = None
            if document_list and len(document_list) > 0:
                # Create filter for filepath
                escaped_docs = [f"'{doc}'" for doc in document_list[:10]]  # Limit to 10 docs
                filter_expression = f"search.in(filepath, {','.join(escaped_docs)})"
